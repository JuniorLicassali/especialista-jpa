...
//                new BigDecimal(499), new BigDecimal(23)));

        criteriaQuery.where(criteriaBuilder.between(root.get(Pedido_.dataCriacao), LocalDateTime.now().minusDays(5), LocalDateTime.now()));
    ...
    public void usarMaiorEMenorComDatas() {
        ...
        criteriaQuery.where(criteriaBuilder.greaterThanOrEqualTo(root.get(Pedido_.dataCriacao), tresDiasAtras));
...
//        criteriaQuery.where(criteriaBuilder.isNull(root.get(Produto_.foto)));
    ...
    @Test
    public void usarExpressoesCondicionaisLike() {
        CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
